...
sjPlot::tab_df(x = as.data.frame(HEB_HEComp), alternate.rows = T,title = "Homoeolog Expression Bias with and without Homoeologous Exchanges Chi Squared table",file="HEB_HEComp_ChiRes.doc",digits = 3)
sjPlot::tab_df(x = as.data.frame(HEB_Test), alternate.rows = T,title = "Homoeolog Expression Bias including Homoeologous Exchanges Chi Squared table",file="HEB_HEsp_ChiRes.doc")
...
TwoTwo_Main_Plot %>% ggplot(aes(y=PRV,x=Class,fill=Class))+geom_jitter()+geom_boxplot(alpha=0.5)+ylab("Polyploid Response Variance")+theme_cowplot(12)+scale_fill_discrete(label=c("Dosage \n Insensitive","Dosage \n Sensitive"))+stat_compare_means(method = "kruskal.test",label="p.format",size=4.5,label.x = 2)
...
TwoTwo_HEB_Plot %>% ggplot(aes(y=PRV,x=Bias.y,fill=Class))+geom_jitter()+geom_boxplot(alpha=0.5)+ylab("Polyploid Response Variance")+theme_cowplot(12)+scale_fill_discrete(label=c("Dosage \n Insensitive","Dosage \n Sensitive"))+stat_compare_means(method = "kruskal.test",label="p.format",size=4.5,label.x = 2)
...
TwoTwo_Gen_Plot<-TwoTwo_w_DosageClassFromGO %>% filter(Indiv != "RS-Parent") %>% separate(Indiv,into=c("Line","Generation"),sep="00") %>% group_by(GO,Class,Generation) %>% mutate(sd_exp_response=sd(SumTPM.x/(SumTPM.y/2))) %>% mutate(avg_exp_response= mean(SumTPM.x/(SumTPM.y/2))) %>% mutate(PRV=sd_exp_response/avg_exp_response) %>% summarise(PRV)  %>% mutate(count=n()) %>% distinct() %>% filter(count>20) 
  ...
  TwoTwo_Gen_Plot%>% ggplot(aes(y=PRV,x=Generation,fill=Class))+geom_jitter()+geom_boxplot(alpha=0.5)+ylab("Polyploid Response Variance")+theme_cowplot(12)+scale_fill_discrete(label=c("Dosage \n Insensitive","Dosage \n Sensitive"))+stat_compare_means(method = "kruskal.test",label="p.format",size=4.5,label.x = 2)+scale_x_discrete(limits=c("S1","S5","S10"))

```



```{r Calculate expression response to different kinds of homoeologous exchange events}
...
HERVMainPlot<- ReanalyHERVMain %>% drop_na(HERV) %>% summarise(avg_exp_response,sd_exp_response,HERV) %>% mutate(count=n()) %>% distinct() %>% filter(count>20) %>% summarise(avgHERV=mean(HERV),count=mean(count))

  HERVMainPlot %>% ggplot(aes(y=avgHERV,x=Class,fill=Class))+geom_jitter()+geom_boxplot(alpha=0.5)+ylab("Homoeologous Exchange Response Variance")+theme_cowplot(12)+scale_fill_discrete(label=c("Dosage \n Insensitive","Dosage \n Sensitive"))+stat_compare_means(method = "kruskal.test",label="p.format",size=4.5,label.x = 2)


#Homoeolog Expression Bias Comparison

ZerotoFour_HERV_HEB<-ZerotoFour_w_DosageClassFromGO  %>% filter(R500TPM.y > 0) %>% filter(!(Indiv == "RS-1100S10" & TO1000Chr.x =="C4")) %>%filter(TO1000Chr.x != "C1" & TO1000Chr.x !="C2" & R500Chr.x !="A10") %>% group_by(GO,Class,Bias.y) %>%  mutate(avg_exp_response=mean(SumTPM.x/((2*R500TPM.y)))) %>% mutate(sd_exp_response=sd(SumTPM.x/((2*R500TPM.y)))) %>% mutate(HERV=sd_exp_response/avg_exp_response) %>% mutate(dosage="0:4")

OnetoThree_HERV_HEB<-OnetoThree_w_DosageClassFromGO   %>% filter(!(Indiv == "RS-1100S10" & TO1000Chr.x =="C4")) %>%filter(TO1000Chr.x != "C1" & TO1000Chr.x !="C2" & R500Chr.x !="A10") %>% group_by(GO,Class,Bias.y) %>%  mutate(avg_exp_response=mean(SumTPM.x/((TO1000TPM.y+3*R500TPM.y)/2))) %>% mutate(sd_exp_response=sd(SumTPM.x/((TO1000TPM.y+3*R500TPM.y)/2))) %>% mutate(HERV=sd_exp_response/avg_exp_response)  %>% mutate(dosage="1:3")
 

ThreetoOne_HERV_HEB<-ThreetoOne_w_DosageClassFromGO %>% filter(!(Indiv == "RS-1100S10" & TO1000Chr.x =="C4")) %>%filter(TO1000Chr.x != "C1" & TO1000Chr.x !="C2" & R500Chr.x !="A10") %>% group_by(GO,Class,Bias.y) %>%   mutate(avg_exp_response=mean(SumTPM.x/((3*TO1000TPM.y+R500TPM.y)/2))) %>% mutate(sd_exp_response=sd(SumTPM.x/((3*TO1000TPM.y+R500TPM.y)/2))) %>% mutate(HERV=sd_exp_response/avg_exp_response)  %>% mutate(dosage="3:1")

FourtoZero_HERV_HEB<-FourtoZero_w_DosageClassFromGO  %>% filter(TO1000TPM.y > 0) %>% filter(!(Indiv == "RS-1100S10" & TO1000Chr.x =="C4")) %>%filter(TO1000Chr.x != "C1" & TO1000Chr.x !="C2" & R500Chr.x !="A10") %>% group_by(GO,Class,Bias.y) %>% mutate(avg_exp_response=mean(SumTPM.x/((2*TO1000TPM.y)))) %>% mutate(sd_exp_response=sd(SumTPM.x/((2*TO1000TPM.y)))) %>% mutate(HERV=sd_exp_response/avg_exp_response)  %>% mutate(dosage="4:0") 
  

ReanalyHERV_HEB<-rbind(ZerotoFour_HERV_HEB,OnetoThree_HERV_HEB,ThreetoOne_HERV_HEB,FourtoZero_HERV_HEB) 

HERV_HEB_Separate <- ReanalyHERV_HEB %>% group_by(GO,Class,Bias.y,dosage) %>% drop_na(HERV) %>% summarise(avg_exp_response,sd_exp_response,HERV,dosage) %>% mutate(count=n()) %>% distinct() 

HERV_HEB_Plot <- ReanalyHERV_HEB %>% drop_na(HERV) %>% summarise(avg_exp_response,sd_exp_response,HERV) %>% mutate(count=n()) %>% distinct() %>% filter(count>20) %>% summarise(avgHERV=mean(HERV),count=mean(count)) 

HERV_HEB_Plot %>% ggplot(aes(y=avgHERV,x=Bias.y,fill=Class))+geom_jitter()+geom_boxplot(alpha=0.5)+ylab("Homoeologous Exchange Response Variance")+theme_cowplot(12)+scale_fill_discrete(label=c("Dosage \n Insensitive","Dosage \n Sensitive"))+stat_compare_means(method = "kruskal.test",label="p.format",size=4.5,label.x = 2)


# Generation Comparison

ZerotoFour_HERV_Gen<-ZerotoFour_w_DosageClassFromGO  %>% filter(R500TPM.y > 0) %>% filter(!(Indiv == "RS-1100S10" & TO1000Chr.x =="C4")) %>%filter(TO1000Chr.x != "C1" & TO1000Chr.x !="C2" & R500Chr.x !="A10")%>% separate(Indiv,into=c("Line","Generation"),sep="00") %>% group_by(GO,Class,Generation) %>%  mutate(avg_exp_response=mean(SumTPM.x/((2*R500TPM.y)))) %>% mutate(sd_exp_response=sd(SumTPM.x/((2*R500TPM.y)))) %>% mutate(HERV=sd_exp_response/avg_exp_response) %>% mutate(dosage="0:4")

OnetoThree_HERV_Gen<-OnetoThree_w_DosageClassFromGO  %>% filter(!(Indiv == "RS-1100S10" & TO1000Chr.x =="C4")) %>%filter(TO1000Chr.x != "C1" & TO1000Chr.x !="C2" & R500Chr.x !="A10")  %>% separate(Indiv,into=c("Line","Generation"),sep="00") %>% group_by(GO,Class,Generation) %>% mutate(avg_exp_response=mean(SumTPM.x/((TO1000TPM.y+3*R500TPM.y)/2))) %>% mutate(sd_exp_response=sd(SumTPM.x/((TO1000TPM.y+3*R500TPM.y)/2))) %>% mutate(HERV=sd_exp_response/avg_exp_response)  %>% mutate(dosage="1:3")

ThreetoOne_HERV_Gen<-ThreetoOne_w_DosageClassFromGO  %>% filter(!(Indiv == "RS-1100S10" & TO1000Chr.x =="C4")) %>%filter(TO1000Chr.x != "C1" & TO1000Chr.x !="C2" & R500Chr.x !="A10")  %>% separate(Indiv,into=c("Line","Generation"),sep="00") %>% group_by(GO,Class,Generation) %>% mutate(avg_exp_response=mean(SumTPM.x/((3*TO1000TPM.y+R500TPM.y)/2))) %>% mutate(sd_exp_response=sd(SumTPM.x/((3*TO1000TPM.y+R500TPM.y)/2))) %>% mutate(HERV=sd_exp_response/avg_exp_response)  %>% mutate(dosage="3:1")

FourtoZero_HERV_Gen<-FourtoZero_w_DosageClassFromGO  %>% filter(TO1000TPM.y > 0) %>% filter(!(Indiv == "RS-1100S10" & TO1000Chr.x =="C4")) %>%filter(TO1000Chr.x != "C1" & TO1000Chr.x !="C2" & R500Chr.x !="A10") %>% separate(Indiv,into=c("Line","Generation"),sep="00") %>% group_by(GO,Class,Generation) %>%  mutate(avg_exp_response=mean(SumTPM.x/((2*TO1000TPM.y)))) %>% mutate(sd_exp_response=sd(SumTPM.x/((2*TO1000TPM.y)))) %>% mutate(HERV=sd_exp_response/avg_exp_response)  %>% mutate(dosage="4:0") 

ReanalyHERV_Gen<-rbind(ZerotoFour_HERV_Gen,OnetoThree_HERV_Gen,ThreetoOne_HERV_Gen,FourtoZero_HERV_Gen) 

HERV_Gen_Separate<- ReanalyHERV_Gen %>% group_by(GO,Class,Generation,dosage) %>% drop_na(HERV) %>% summarise(avg_exp_response,sd_exp_response,HERV,dosage) %>% mutate(count=n()) %>% distinct() 

HERV_Gen_Plot<-ReanalyHERV_Gen %>% drop_na(HERV) %>% summarise(HERV) %>% mutate(count=n()) %>% distinct() %>% filter(count>20) %>% summarise(avgHERV=mean(HERV),count=mean(count))

HERV_Gen_Plot %>% ggplot(aes(y=avgHERV,x=Generation,fill=Class))+geom_jitter()+geom_boxplot(alpha=0.5)+ylab("Homoeologous Exchange Response Variance")+theme_cowplot(12)+scale_fill_discrete(label=c("Dosage \n Insensitive","Dosage \n Sensitive"))+stat_compare_means(method = "kruskal.test",label="p.format",size=4.5,label.x = 2) +scale_x_discrete(limits=c("S1","S5","S10"))

```

```{r redo compare HERV and PRV}
...
TwoTwo_w_DosageClassFromGO %>% filter(Indiv != "RS-Parent") %>% group_by(GO,Class) %>% summarise(SumTPM.x) %>% mutate(count=n()) %>% distinct() %>% filter(count>20) %>% summarise(avgSumTPM=mean(SumTPM.x)) %>% ggplot(data=.,aes(x=Class,y=avgSumTPM,fill=Class))+geom_boxplot(alpha=0.5)+ylim(0,400)+theme_cowplot(12)+stat_compare_means(method = "kruskal.test",label="p.format",size=4.5,label.x = 2)
...
ReanalyHERVMain %>% group_by(GO,Class) %>% summarise(SumTPM.x) %>% mutate(count=n()) %>% distinct() %>% filter(count>20) %>% summarise(avgSumTPM=mean(SumTPM.x)) %>% ggplot(data=.,aes(x=Class,y=avgSumTPM,fill=Class))+geom_boxplot(alpha=0.5)+ylim(0,400)+theme_cowplot(12)+stat_compare_means(method = "kruskal.test",label="p.format",size=4.5,label.x = 2)
...
#Compare HERV and PRV overall
Plot_1<-ReanalyHERVMain %>% drop_na(HERV) %>% summarise(HERV) %>% mutate(count_HE=n()) %>% distinct() %>% filter(count_HE>20) %>% mutate(HERV=mean(HERV)) %>% distinct(GO,Class, HERV,count_HE)
...
ggplot(data=Plot_test,aes(x=Type,y=Variance,group=Type,fill=Type))+geom_jitter()+geom_boxplot(alpha=0.5)+theme_cowplot(12)+stat_compare_means(method = "kruskal.test",label="p.format",size=4.5,label.x = 2)+ylab("Coeficient of Variance")
 
##############################################################
#Compare HERV and PRV by homoeolog expression bias
Plot_1<-ReanalyHERV_HEB  %>% drop_na(HERV) %>% summarise(HERV) %>% mutate(count_HE=n()) %>% distinct() %>% filter(count_HE>20) %>% mutate(HERV=mean(HERV)) %>% distinct(GO,Class, Bias.y,HERV,count_HE) 
...
#Compare HERV and PRV by generation
Plot_1<-ReanalyHERV_Gen  %>% drop_na(HERV) %>% summarise(HERV) %>% mutate(count_HE=n()) %>% distinct() %>% filter(count_HE>20) %>% mutate(HERV=mean(HERV)) %>% distinct(GO,Class, Generation,HERV,count_HE) 
